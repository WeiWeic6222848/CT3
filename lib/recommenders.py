...
class Cache(object):
    ...
    def store(self):
        ...
        if not self._loaded_file:
            filename = re.sub("[^0-9a-zA-Z=.,]+", "", id) + "_" + str(
                int(time.time())) + ".npz"
            self._models[id] = filename
        else:
            filename = self._loaded_file

        np.savez(self.base_path + filename, **self._instance.get_params(),
                 id=self._instance.get_id())
...
class ScoreTable(object):
    ...
    def to_dict(self):
        return {"best_param": self.best_param(),
...
class Recommender(object):
    ...
    @classmethod
    def grid_search(cls, implicit, supervised_train, supervised_val, metric,
                    verbose=False):
        ...
        for param_id, params in enumerate(product_dict(cls.PARAM_GRID)):
            ...
            model.fit()
            val_score = \
                list(model.evaluate(supervised_val, [metric]).values())[0][
                    "mean"]
    ...
    def print_scored_candidates(self, item_id, candidate_ids=None, k=10):
        candidate_ids, scores = self.score_candidates(item_id, candidate_ids,
                                                      rank=True)
    ...
    def items_to_df(self, movie_id, candidate_ids, scores):
        ...
        return pd.DataFrame(
            {"title": [titles[m] for m in candidate_ids], "score": scores,
             "movieId": candidate_ids,
             "poprank": [popranks[m] for m in candidate_ids]})
    ...
    def create_debug_file(self, rankings, k=10,
        ...
        for _, ranking in rankings.iterrows():
            ...
            df['c_ab'] = cc.counts_ab(ranking["a_iid"],
                                      np.arange(self._data.n_items))
            ...
            for c in ["rank", "relevance", "year", "time_first", "popularity",
                      "counts(a|b)"]:
                report[c] = report[c].astype("Int64")

            reports += [report.to_string(
                formatters={"title": lambda x: (str(x) + " " * 50)[:35]},
                index=False)]
        with open(self.__class__.__name__ + ".debug.txt", 'w',
...
class CachedRecommender(Recommender):
    ...
    def fit(self):
        ...
        if cached_params is None:
            ...
            cached_params = {k: v for k, v in cached_params.items() if
                             k != "id"}
...
class ItemKNN(Cooccur):
    ...
    def _score_candidates(self, item_id, candidate_ids):
        ...
        candidate_norms = np.power(self._norms[candidate_ids] + lmbda,
                                   1.0 - alpha)
...
class BPR(CachedRecommender):
    ...
    def _fit(self):
        self._model = implicit.bpr.BayesianPersonalizedRanking(
            regularization=self._my_settings["lambda"],
            factors=self._my_settings["n_factors"])
...
class SVD(BPR):
    ...
    def __init__(self, data, supervised_data=None, n_factors=10,
                 center_data=False):
    ...
    def _fit(self):
        if self._my_settings["center_data"]:
            ...
            self._item_factors = self._model.fit_transform(
                self._ratings.todense())
        else:
...
class Genre(CosineNN):
    def _fit(self):
        ...
        self._item_factors = items.loc[
            np.arange(items.index.max() + 1, dtype=int),
...
class DebiasedModel(Recommender):
    # modified param grid for param optimization - CT3
    PARAM_GRID = {'lr': [0.005, 0.01, 0.02],
                  'n_neg': [200, 500, 700, 1000]}

    # modified init for param optimization - CT3
    def __init__(self, data, labeled_rankings, model="ranker", n_neg=500,
                 lr=0.02):
        self._data = data
    ...
    # modified this functions parameter for optimization - CT3
    def _add_negative_samples(self, rankings, n_neg=None):
        if n_neg is None:
            n_neg = self.n_neg
        ...
        def add_negative(grp):
            ...
            negative_ids = np.setdiff1d(np.arange(self._data.n_items),
                                        positive_ids)
            neg_samples = np.random.choice(negative_ids, n_neg, replace=False)

            neg_df = pd.DataFrame({"a_iid": [grp.name] * len(neg_samples),
                                   "b_iid": neg_samples,
                                   "label": np.zeros_like(neg_samples)})
            df = pd.concat([pos, neg_df])
            return df

        return rankings.groupby("a_iid").apply(add_negative).reset_index(
            drop=True)
    ...
    def _learn_propensities(self):
        ...
        X, y, offset = prop.RankingToPairs(column_mapping=cols).transform(
            rankings)
        print("Transformed to %d pairs" % len(y))

        # this is the network that fits for the losses, modified lr for param optimization - CT3
        ranker = prop.Ranker(model="linear", lr=self.lr, n_steps=300)
    ...
    def debug(self, rankings, examples_to_show=0):
        for name, grp in itertools.islice(rankings.groupby("a_iid"),
                                          examples_to_show):
            ...
            print(my_grp[["b_name", "label", "MLE_IPS_rank", "MLE_est",
                          "p_inv_pred"] +
                         ["counts_ab", "counts_b"]].sort_values("label",
    ...
    def _get_features(self, item_id, candidate_ids):
        ...
        cc = self._cc
        features += [
            cc.counts_ab(item_id, candidate_ids) / cc.counts_b(candidate_ids)]
...
class SklearnRec(DebiasedModel):
    ...
    def __init__(self, data, labeled_rankings, method="random",
                 n_estimators=100, max_depth=3):
        super().__init__(data, labeled_rankings)
        self._data = data
        self._cc = Cooccur(self._data)
        self._cc.fit()
        self._method = method.lower()
        self._labeled_rankings = labeled_rankings
        self._my_settings = {"n_estimators": n_estimators,
                             "max_depth": max_depth}
        self._raw_features = ["popularity_log", "year", "time_first"]
    ...
    def _fit(self):
        ...
        ranker = sklearn.ensemble.GradientBoostingClassifier(random_state=0,
                                                             **self._my_settings)
