...
def read_movielens_ratings(movielens_file):
    ...
    timestamps = sp.coo_matrix(
        (ratings_raw["timestamp"].values, (i, j))).transpose()
...
class ImplicitMovieLens(object):
    def __init__(self, version):
        ...
        if os.path.exists(f_ratings_npz):
            ...
            ratings_raw = pd.read_csv(f_ratings,
        ...
        print("Read %d x %d user-item matrix" % (
        ...
        items["normalized_popularity"] = (
                items["popularity"] / (
                items["year"].max() + 1 - items["year"]))
        items["popularity_rank"] = items["popularity"].rank(method="first",
                                                            ascending=False)
    ...
    # modified data loader for varying size, added limitAmount - CT3
    def load_rankings(self, file, a_col, b_col, label_col, verbose=False,
                      limitAmount=None):

        rankings = pd.read_csv(file)
        if limitAmount != None:
            if len(rankings) > limitAmount:
                # randomly delete rankings until its under the limit.
                drop_indices = np.random.choice(rankings.index,
                                                len(rankings) - limitAmount ,
                                                replace=False)
                rankings = rankings.drop(drop_indices)
        ...
        valid_rankings = rankings[
            (rankings.a_iid > -1) & (rankings.b_iid > -1)]

        info = collections.OrderedDict()
        info["# rankings"] = rankings[a_col].nunique()
        info["# invalid rankings"] = (
                rankings.groupby(by=a_col).max()["a_iid"] == -1).sum()
        info["# invalid results"] = (rankings["b_iid"] == -1).sum()
        per_ranking = valid_rankings.groupby(by="a_iid").count()["b_iid"]
        labels_per_ranking = valid_rankings.groupby(by="a_iid").sum()[
            label_col]
        ...
        info["Pos labels in each ranking"] = ("%.2f (min=%d, max=%d))" % (
            labels_per_ranking.mean(), labels_per_ranking.min(),
            labels_per_ranking.max()))
        if verbose:
            print(info)

        df = pd.DataFrame({"a_iid": valid_rankings["a_iid"],
                           "a_ml_id": valid_rankings[a_col],
                           "b_iid": valid_rankings["b_iid"],
                           "b_ml_id": valid_rankings[b_col],
                           "label": valid_rankings[label_col]})

        As, Bs = list(), list()
        for name, group in df.groupby("a_iid"):
            labels = np.zeros(self.n_items)
            labels[group["b_iid"].values.flatten()] = group[
                "label"].values.flatten()
            As.append(name)
            Bs.append(labels)
...
def extract_timestamp_features(timestamps, remaining_movie_ids):
    ...
    for i, mid in enumerate(remaining_movie_ids):
        ...
        results += [{"movieId": mid, "time_peak": peak,
                     "time_first": first, "time_skew": skew,
                     "tm_var": coeffvar}]
...
def convert_to_implicit(version, rating_threshold=3.0, min_freq=30):
    ...
    titles["year"] = titles["title"].str.extract(r'\((\d{4})\)').fillna(
        "-1").astype(int)
    timestamp_features = extract_timestamp_features(timestamps,
                                                    remaining_movie_ids)
...
if __name__ == "__main__":
    ...
    parser.add_argument("movielens_version", type=str,
                        choices=["100k", "20m", "25m"],
    ...
    convert_to_implicit(args.movielens_version, args.min_rating,
                        args.min_count)
